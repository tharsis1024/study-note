...
```
首先保存上下文,包括寄存器值,程序计数器,堆栈指针等
切换下一个上下文,加载该上下文的执行状态
更新内存管理数据结构,任务控制块等
切换完成,继续执行程序
```

进程只有一个唯一非零正整数ID,通过getpid获取进程的PID,通过getppid获取进程父进程的PID 

进程的三种状态

```
运行:在cpu执行或者等待执行
停止:进程被挂起且不会被调度,当收到SIGSTOP,SIGTSTP,SIGTTIN或者SIGTTOU时,进程就停止,直到进程收到SIGCONT
终止:进程永远停止:1收到信号,该信号默认终止信号 2.从主程序返回 3.调用exit函数
```

exit函数以status退出状态终止进程



fork进程可以创建一个新的运行的子进程,子进程会得到父进程用户级虚拟地址空间相同(但独立)的的副本,包括了代码,,数据段,堆,栈,共享库,子进程可以读写父进程中打开的任何文件,两者最大的区别是不同的PID

fork的几个点

```
调用一次返回两次:fork被调用一次但是会返回两次,一次是父进程中,一次是子进程中,在父进程中,fork返回子进程的PID,在子进程中,fork返回0
并发执行:父进程和子进程是并发运行的独立进程,内核能以任何方式交替执行逻辑流中的指令
相同但是独立的地址空间:在调用fork并返回后的时候立即暂停进程,将会看到两个进程的空间是完全相同的
共享文件: 子进程和父进程会共享文件
```



当进程被终止时,内核并不会立即清除,而是会保持已终止的状态直到父进程进行回收,当父进程回收已终止的子进程时,内核将子进程的退出状态传给父进程,然后抛弃该进程,被终止但未回收的进程被称为僵尸进程

如果父进程被终止,它的子进程会成为孤儿进程,孤儿进程将会被init进行收养
